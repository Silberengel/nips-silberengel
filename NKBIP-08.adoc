...
Clients MUST normalize link fields before parsing and searching. Normalize according to NIP-54: remove quotes, convert non-alphanumeric characters (except hyphens) to hyphens, lowercase all text. Versions separated by commas (e.g., `| kjv, niv`) should have commas normalized to spaces.
...
* `IDENTIFIER` matches letters, numbers, hyphens, underscores (normalized to lowercase+hyphens for tags)
